...
// ⚠️ رابط قاعدة البيانات (Firebase Realtime DB) لجعله عالمي وحقيقي للكل
// سأعلمك لاحقاً كيف تنشئ واحدة مجانية وتضع رابطها هنا
...
async function recordRealVisit() {
  ...
  try {
    ...
    // إذا لم تقم بربط القاعدة بعد، سيستخدم هذا النظام كبديل مؤقت
...
async function saveUserGlobal(userObj) {
  ...
  // الحفظ العالمي في قاعدة البيانات
  try { 
    await fetch(`${DB_URL}/users.json`, { 
      method: 'POST', 
      body: JSON.stringify(userObj) 
    }); 
  } catch(e) { console.log("Database not linked yet."); }
}
...
function renderAuth() {
  ...
  // الجلسة محفوظة في localStorage فلا تضيع بالتحديث نهائياً
  if (session) {
...
async function doLogin() {
  ...
  // فحص باقي المستخدمين من القاعدة العالمية (أو المحلية)
...
async function loadPanelData() {
  // جلب البيانات من القاعدة (إذا مربوطة) أو المحلي
  const users = await getUsers();
  ...
  uList.innerHTML = users.length ? users.reverse().map(u => {
...
async function submitOrder() {
  ...
  try {
    ...
    console.log("Database not linked yet.");
  }

  updateOrderCount();
  fetchRealOrders(); // تحديث الرقم فوراً للكل
...
// ===== TRANSLATIONS (اللغات - تم ضغطها لسرعة الأداء) =====
const T = {
