...
window.onload = async () => {
  ...
  // تشغيل وتحديث عداد الطلبات كل 15 ثانية
  refreshOrderCount();
...
// ============================
// 4. AUTH RENDER & LOGOUT (واجهة الحساب)
// ============================
function renderAuth(){
  const t = T[lang] || T.ar;
...
// ============================
// 6. AUTH LOGIC (تسجيل الدخول والإنشاء)
// ============================
...
// ============================
// 7. DISCOUNT LOGIC (نظام أكواد الخصم)
// ============================
...
async function loadDiscounts(){
  ...
  if(pD) pD.textContent = entries.length;
  ...
  list.innerHTML = entries.map(([k, d]) => `
    <div class="dc" style="background:var(--surf); border:1px solid rgba(16,185,129,.2); border-radius:9px; padding:10px 14px; margin-bottom:6px; display:flex; justify-content:space-between; align-items:center;">
      <span style="font-size:13px; font-weight:700;">🏷️ <strong style="color:var(--grn)">${d.code}</strong> — خصم <strong style="color:var(--acc3)">${d.pct}%</strong></span>
      <button class="del-btn" style="background:rgba(239,68,68,.12); border:1px solid rgba(239,68,68,.25); color:var(--red); padding:3px 10px; border-radius:6px; font-size:11px; cursor:pointer;" onclick="delDiscount('${k}')">🗑️ حذف</button>
    </div>`).join('');
}
...
// ============================
// 8. ORDERING LOGIC (إرسال الطلبات)
// ============================
...
async function showPanel(){
  ...
  const pD = document.getElementById('pD');
  if(pD) pD.textContent = Object.keys(discountsObj).length;

  document.getElementById('pOL').innerHTML = orders.length
    ? orders.slice().reverse().map(o => `<div class="pc" style="background:var(--surf); border:1px solid var(--bdr); border-radius:9px; padding:12px; margin-bottom:8px;"><h4>📦 ${o.bot} — ${o.date}</h4><p>👤 <strong>${o.name}</strong> | 📞 <strong>${o.contact}</strong><br>💰 <strong>${o.budget||'—'}</strong> | 💳 <strong>${o.pay||'—'}</strong>${o.discountCode && o.discountCode !== '—' ? ` | 🏷️ <strong style="color:var(--grn)">${o.discountCode} (${o.discountPct}%)</strong>` : ''}<br>📱 <span style="font-size:10px">${(o.device||'').substring(0,90)}</span>${o.desc && o.desc !== '—' ? '<br>📝 ' + o.desc : ''}</p></div>`).join('')
  ...
  document.getElementById('pUL').innerHTML = users.length
    ? users.map(u => {
      ...
      return `<div class="uc" style="display:flex; align-items:flex-start; gap:10px; background:var(--surf); border:1px solid var(--bdr); border-radius:9px; padding:10px 14px; margin-bottom:6px;"><div class="ua" style="width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg, var(--acc), var(--acc2));display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;margin-top:2px;">${isOw?'👑':'👤'}</div><div class="ui"><h4>${u.name} <span class="utg ${isOw?'ow':'us'}" style="display:inline-block; padding:2px 8px; border-radius:10px; font-size:10px; font-weight:700; margin-bottom:4px; ${isOw?'background:rgba(245,158,11,.15);border:1px solid rgba(245,158,11,.3);color:var(--ow)':'background:rgba(124,58,237,.12);border:1px solid rgba(124,58,237,.25);color:#a78bfa'}">${isOw?'👑 Owner':'عضو'}</span></h4><p style="font-size:11px;color:var(--mut);line-height:1.6;word-break:break-all;">📧 ${u.email}<br>🔒 <span style="font-family:monospace;font-size:10px">${(u.hash||'').substring(0,40)}…</span><br>📱 <span style="font-size:10px">${(u.device||'').substring(0,80)}</span><br>📅 ${dt}</p></div></div>`;
    }).join('')
...
function panelTab(tab, btn){
  ...
  if(pDL) pDL.style.display = tab === 'd' ? 'block' : 'none';
  if(tab === 'd') loadDiscounts();
}

// ============================
// 10. UI & MODALS & TRANSLATIONS
// ============================
